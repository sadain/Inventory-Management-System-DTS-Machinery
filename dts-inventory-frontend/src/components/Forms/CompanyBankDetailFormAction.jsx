...
import { useCompanyBankDetailMutation, useCompanyBankDetailEditMutation } from "api/useApi"
import { z } from "zod";
...
const CompanyBankDetailFormAction = (props) => {
  const { type, queryClient, companyBankDetailRow, companyBankDetailId } = props || {};
  ...
  const { mutate: companyBankDetailMutate } = useCompanyBankDetailMutation({
  ...
  const { mutate: companyBankDetailEditMutate } = useCompanyBankDetailEditMutation({
    onSuccess: (data, variables, context) => {
      setShow(false);
      queryClient.invalidateQueries({
        queryKey: ["allCompanyBanksData"],
  ...
  const {
    ...
    defaultValues: {
      ...
      bankBranch: companyBankDetailRow ? companyBankDetailRow.bankBranch : "",
  ...
  const onSubmit = (data) => {
    ...
    if (companyBankDetailRow?.id !== undefined) {
      companyBankDetailEditMutate({ id: companyBankDetailRow.id, ...reqData });
    } else {
      companyBankDetailMutate(reqData);
      reset();
    }
  };
  ...
  return (
    <div>
      ...
      <Modal show={show} onHide={handleClose} centered className="dialog">
        ...
        <Form
          onSubmit={handleSubmit((data) => onSubmit(data, companyBankDetailRow?.id))}
        >
          <Modal.Body>
...
export default CompanyBankDetailFormAction;

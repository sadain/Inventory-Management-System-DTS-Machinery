...
import { useExpenseDeleteMutation } from "api/useApi"

const { Group } = Form;

const ExpenseFormDelete = (props) => {
  const { type, queryClient, expenseRow } = props || {};
  ...
  const { mutate: expenseDeleteMutate } = useExpenseDeleteMutation({
    ...
    onError: (error) => {
      if (error.includes("REFERENCE constraint")) {
        toast.error("Can't delete this Expense as it is used in another screen");
      } else {
        toast.error(error);
  ...
  const {
    ...
    defaultValues: {
      ...
      exchangeRate: expenseRow ? expenseRow.exchangeRate : "",
    },
  });

  const onSubmit = () => {
    expenseDeleteMutate(expenseRow);
  };
  ...
  return (
    <div>
      ...
      <Modal show={show} onHide={handleClose} centered className="dialog">
        ...
        <Form onSubmit={handleSubmit((data) => onSubmit(data, expenseRow?.id))}>
          <Modal.Body>
            <Group className="form-floating">
              <p>
                Are you sure you want to delete record name: "{expenseRow.particulars}"
                ?
              </p>
            </Group>
          </Modal.Body>
...
export default ExpenseFormDelete;

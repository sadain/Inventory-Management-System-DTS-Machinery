...
import { useProformaInvoiceDeleteMutation } from "api/useApi";

const { Group } = Form;

const ProformaInvoiceFormDelete = (props) => {
  const { type, queryClient, proformaInvoiceRow } = props || {};

  const {id, quotationNo } = proformaInvoiceRow;
  ...
  const {mutate: proformaInvoiceDeleteMutate} = useProformaInvoiceDeleteMutation({
    ...
    onError: (error) => {
      if (error.includes("REFERENCE constraint")) {
        toast.error("Can't delete this proforma invoice as it is used in another screen");
      } else {
        toast.error(error);
...
export default ProformaInvoiceFormDelete;

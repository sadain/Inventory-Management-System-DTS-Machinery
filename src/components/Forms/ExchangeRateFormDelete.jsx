...
import { useExchangeRateDeleteMutation } from "api/useApi";


const { Group } = Form;

const ExchangeRateFormDelete = (props) => {
  const { type, queryClient, exchangeRateRow } = props || {};
  ...
  const { mutate: exchangeRateDeleteMutate } = useExchangeRateDeleteMutation({
    ...
    onError: (error) => {
      if (error.includes("REFERENCE constraint")) {
        toast.error("Can't delete this exchange rate as it is used in another screen");
      } else {
        toast.error(error);
  ...
  const onSubmit = () => {
    exchangeRateDeleteMutate(exchangeRateRow);
  };
  ...
  return (
    <div>
      ...
      <Modal show={show} onHide={handleClose} centered className="dialog">
        ...
        <Form onSubmit={handleSubmit((data) => onSubmit(data, exchangeRateRow?.id))}>
          <Modal.Body>
            <Group className="form-floating">
              <p>
                Are you sure you want to delete this currency: "{exchangeRateRow.currencyName}"
                ?
              </p>
            </Group>
          </Modal.Body>
...
export default ExchangeRateFormDelete;

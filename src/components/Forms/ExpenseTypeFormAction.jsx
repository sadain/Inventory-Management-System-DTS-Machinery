...
import { useExpenseTypeMutation, useExpenseTypeEditMutation } from "api/useApi";
...
const ExpenseTypeFormAction = (props) => {
    ...
    const { mutate: expenseTypeMutate } = useExpenseTypeMutation({
      onSuccess: () => {
        setShow(false);
    ...
    const { mutate: expenseTypeEditMutate } = useExpenseTypeEditMutation({
    ...
    const onSubmit = (data, id) => {
      ...
      if (expenseTypeRow?.id !== undefined) {
        expenseTypeEditMutate({ id: expenseTypeRow.id, ...reqData });
      } else {
        expenseTypeMutate(reqData);
        reset();
      }
    };
    ...
    return (
        <div>
          ...
          <Modal show={show} onHide={handleClose} centered className="dialog">
            ...
            <Form
              onSubmit={handleSubmit((data) => onSubmit(data, expenseTypeRow?.id))}
            >
              <Modal.Body>
                <Group className="form-floating">
                  ...
                  <Label htmlFor="inputName">Expense Type</Label>
...
export default ExpenseTypeFormAction